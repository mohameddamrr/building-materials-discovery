...
export function RecommendationList({ recommendations }: { recommendations: ResolvedRecommendation[] }) {
  return (
    <section aria-labelledby="recommendations-heading" className="scroll-mt-6 mt-10" id="products-to-explore">
      ...
      <ul className="mt-7 grid gap-5 md:grid-cols-2">{recommendations.map(({ product, reason }) => <li key={product.id}><article className="group h-full overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-slate-200/80 motion-safe:transition motion-safe:hover:-translate-y-1 hover:shadow-xl"><div className="relative h-40 overflow-hidden bg-slate-900"><img alt="" aria-hidden="true" className="size-full object-cover opacity-80 motion-safe:transition motion-safe:duration-500 motion-safe:group-hover:scale-105" src={productImages[product.slug]} /><div className="absolute inset-0 bg-gradient-to-t from-slate-950/80 to-transparent" /><p className="absolute bottom-4 left-5 text-xs font-bold uppercase tracking-[0.16em] text-amber-300">{categoryLabels[product.category]}</p></div><div className="p-5 sm:p-6"><h3 className="text-xl font-bold text-slate-950">{product.name}</h3><p className="mt-3 text-sm leading-6 text-slate-600">{reason}</p><Link aria-label={`View ${product.name} details`} className="mt-5 inline-flex font-bold text-slate-950 underline decoration-amber-500 decoration-2 underline-offset-4" to={`/products/${product.slug}`}>Explore product <span aria-hidden="true">→</span></Link></div></article></li>)}</ul>
    </section>
  );
}
